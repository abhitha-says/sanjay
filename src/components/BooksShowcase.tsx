...
export interface BookCfg {
  ...
  spineInk?: string;
  spineFont?: string;
  /** Turnable content pages, in order. Page 1 is `pages[0]`. */
  pages?: BookPage[];
}
...
export interface BookPage {
  ...
  title: string;
  body: string;
  /** Optional image drawn onto the page once loaded. */
  image?: string;
}
...
export function BooksShowcase({
  ...
  useEffect(() => {
    ...
    // Draws one turnable content page: period, title, optional photo, body copy.
    // Called twice when an image is supplied — once synchronously (text only,
    // so the page has content immediately) and again once the image decodes.
    function paintContentPage(
      x: CanvasRenderingContext2D,
      w: number,
      h: number,
      page: BookPage,
      ...
      x.fillStyle = '#f4efdf';
      x.fillRect(0, 0, w, h);
      x.fillStyle = 'rgba(130,110,80,0.06)';
      for (let i = 0; i < 1200; i++) x.fillRect(Math.random() * w, Math.random() * h, 1.1, 1.1);

      x.textAlign = 'left';
      x.fillStyle = '#8a6a4a';
      x.font = '700 30px Arial';
      x.fillText(page.period.toUpperCase(), 110, 140);

      x.textAlign = 'right';
      x.fillStyle = '#5d5043';
      x.font = '500 26px Georgia';
      x.fillText(String(pageNum).padStart(2, '0') + ' / ' + String(total).padStart(2, '0'), w - 110, 140);
      x.textAlign = 'left';

      x.globalAlpha = 0.25;
      x.fillStyle = '#2f2a23';
      x.fillRect(110, 168, w - 220, 3);
      x.globalAlpha = 1;

      x.fillStyle = '#2f2a23';
      x.font = '700 64px Georgia';
      const titleLines = wrapText(x, page.title, w - 220);
      let y = 260;
      titleLines.forEach((line) => {
        x.fillText(line, 110, y);
        y += 72;
      });
      y += 20;
      ...
      if (img && img.width && img.height) {
        ...
        x.shadowBlur = 24;
        x.shadowOffsetY = 10;
        x.drawImage(img, ix, y, iw, ih);
        x.restore();
        x.strokeStyle = 'rgba(47,42,35,0.35)';
        x.lineWidth = 3;
        x.strokeRect(ix, y, iw, ih);
        contentBottom = y + ih + 48;
      }

      x.fillStyle = '#4a4136';
      x.font = '400 34px Georgia';
      let by = contentBottom + 6;
      wrapText(x, page.body, w - 220).forEach((line) => {
        x.fillText(line, 110, by);
        by += 48;
      });
      ...
      x.globalAlpha = 0.5;
      x.fillStyle = '#5d5043';
      x.font = '500 24px Georgia';
      x.fillText('— ' + String(pageNum).padStart(2, '0') + ' —', w / 2, h - 70);
      x.globalAlpha = 1;
      x.textAlign = 'left';
    }

    function makeContentPageTex(page: BookPage, pageNum: number, total: number) {
      const w = 1024,
        h = 1536,
        c = mkCanvas(w, h),
        x = c.getContext('2d')!;
      paintContentPage(x, w, h, page, pageNum, total, null);
      const texture = tex(c);
      if (page.image) {
        const img = new Image();
        img.onload = () => {
          if (cancelled) return;
          paintContentPage(x, w, h, page, pageNum, total, img);
          texture.needsUpdate = true;
        };
        img.onerror = () => console.warn('Page image failed to load, kept text-only page:', page.image);
        img.src = page.image;
      }
      return texture;
    }
    ...
    turnPageRef.current = (dir) => {
      if (state.selected) turnPage(state.selected, dir);
    };
    ...
    function open(book: Book | null) {
      ...
      book.pageIndex = 0;
      book.pageTurn.forEach((s) => s.set(0));
      setPageInfo({ index: 0, total: book.pageCount });
  ...
  const handleReadChapter = () => {
    if (selectedCfg?.to) navigate(selectedCfg.to);
  };
  ...
  return (
    <div
      ...
      data-state={uiMode}
      className={cn(
        'book-showcase relative isolate h-full min-h-[560px] overflow-hidden font-sans outline-none [-webkit-tap-highlight-color:transparent]',
    ...
    >
      ...
      {showNav && (
        <nav
          ...
          className={cn(
            'pointer-events-none absolute inset-x-0 top-0 z-40 flex items-center justify-between px-[clamp(20px,4vw,42px)] py-[clamp(18px,3vh,26px)] transition-opacity duration-300',
            uiMode === 'hero' ? 'visible opacity-100' : 'invisible opacity-0',
          )}
        >
          <div className="text-[clamp(20px,2.2vw,29px)] font-extrabold tracking-[-0.01em] text-current">
            {navTitle}
          </div>
        </nav>
      )}
